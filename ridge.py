...
class _BaseRidge(six.with_metaclass(ABCMeta, LinearModel)):
    ...
    def fit_file(self, X, y, sample_weight=None):
        X, y = check_X_y(X, y, ['csr', 'csc', 'coo'], dtype=np.float64,
                         multi_output=True, y_numeric=True)
        ...
        # temporary fix for fitting the intercept with sparse data using 'sag'
        if sparse.issparse(X) and self.fit_intercept:
            self.coef_, self.n_iter_, self.intercept_ = ridge_regression_file(
                X, y, alpha=self.alpha, sample_weight=sample_weight,
                max_iter=self.max_iter, tol=self.tol, solver=self.solver,
                random_state=self.random_state, return_n_iter=True,
                return_intercept=True)
            ...
            self.coef_, self.n_iter_ = ridge_regression_file(
                X, y, alpha=self.alpha, sample_weight=sample_weight,
                max_iter=self.max_iter, tol=self.tol, solver=self.solver,
                random_state=self.random_state, return_n_iter=True,
                return_intercept=False)
...
class Ridge(_BaseRidge, RegressorMixin):
    ...
    def fit_file(self, X, y, sample_weight=None):
        ...
        self : returns an instance of self.
        """
        return super(Ridge, self).fit_file(X, y, sample_weight=sample_weight)
